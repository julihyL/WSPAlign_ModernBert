...
OUTPUT_DIR="$EXPERIMENT_DIR/modernbert_1024_1gpu"
LOG_FILE="$EXPERIMENT_DIR/modernbert_1024_1gpu.log"
...
# The physical GPU ID can be overridden when launching the script.
#
# Example:
# GPU_ID=6 bash pretrain_modernbert_1024_1gpu.sh
GPU_ID="${GPU_ID:-6}"
export CUDA_VISIBLE_DEVICES="$GPU_ID"

# The target number of epochs can be set to 1, 2, or 3.
#
# Examples:
# NUM_EPOCHS=1 bash pretrain_modernbert_1024_1gpu.sh
# NUM_EPOCHS=2 bash pretrain_modernbert_1024_1gpu.sh
# NUM_EPOCHS=3 bash pretrain_modernbert_1024_1gpu.sh
...
# Checkpoint, logging, and warmup configuration.
WARMUP_STEPS="${WARMUP_STEPS:-50}"
SAVE_STEPS="${SAVE_STEPS:-100}"
LOGGING_STEPS="${LOGGING_STEPS:-20}"

# Allow only one, two, or three epochs.
if [[ ! "$NUM_EPOCHS" =~ ^[123]$ ]]; then
    echo "ERROR: NUM_EPOCHS must be 1, 2, or 3." >&2
    exit 1
fi
...
echo "============================================================"
echo "ModernBERT 1024-token training"
...
echo "Effective batch size: $((PER_GPU_BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS))"
echo "Learning rate: $LEARNING_RATE"
echo "Warmup steps: $WARMUP_STEPS"
echo "Checkpoint interval: $SAVE_STEPS"
echo "Logging interval: $LOGGING_STEPS"
echo "============================================================"
...
# Confirm that exactly one CUDA device is visible.
python - <<'PY'
import torch
...
if gpu_count != 1:
    raise RuntimeError(
        f"Expected exactly 1 visible GPU, but found {gpu_count}."
    )
PY
...
# Append run information instead of overwriting the existing log.
{
    ...
    echo "Physical GPU: $GPU_ID"
    echo "Maximum sequence length: 1024"
    echo "Document stride: 256"
    echo "Number of epochs: $NUM_EPOCHS"
    echo "Per-GPU batch size: $PER_GPU_BATCH_SIZE"
    echo "Gradient accumulation steps: $GRADIENT_ACCUMULATION_STEPS"
    echo "Effective batch size: $((PER_GPU_BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS))"
