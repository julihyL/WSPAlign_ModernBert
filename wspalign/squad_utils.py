...
def special_handle_tokenize(doc_tokens, context_text):
    """Handle the SentencePiece `▁` whitespace marker safely."""

    if not doc_tokens or not context_text:
        return doc_tokens

    if "▁" in doc_tokens[0] and not context_text.startswith(" "):
        if doc_tokens[0] == "▁":
            del doc_tokens[0]
        elif doc_tokens[0].startswith("▁"):
            doc_tokens[0] = doc_tokens[0][1:]
        else:
            raise NotImplementedError

    # Removing a standalone `▁` may leave the list empty.
    if not doc_tokens:
        return doc_tokens
    ...
    tmp_context_text = context_text

    # Check that the string is non-empty before accessing [-1].
    while tmp_context_text and tmp_context_text[-1] == " ":
        doc_tokens[-1] += " "
...
def squad_convert_example_to_features(
    ...
    for i, token in enumerate(example.doc_tokens):
        ...
        # This legacy handling is only needed for
        # SentencePiece-style tokenizers whose tokens contain `▁`.
        if (
            example.lang not in divide_by_whitespace_langs
            and is_training
            and any(
                "▁" in sub_token
            ...
            sub_tokens = special_handle_tokenize(
                sub_tokens,
                token,
            )
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        # `texts` and `pairs` already contain final subword tokens.
        # Convert them directly to IDs to prevent a second tokenization.
        text_ids = tokenizer.convert_tokens_to_ids(texts)
        pair_ids = tokenizer.convert_tokens_to_ids(pairs)
        ...
        if expected_length > max_seq_length:
            raise ValueError(
                f"Internal sequence-length error for qas_id={example.qas_id}: "
                f"text={len(text_ids)}, "
                f"pair={len(pair_ids)}, "
                f"special={sequence_pair_added_tokens}, "
                f"total={expected_length}, "
...
class MySquadExample(object):
    ...
    @staticmethod
    def special_handle_tokenize(doc_tokens, context_text):
        return special_handle_tokenize(
            doc_tokens,
            context_text,
        )
